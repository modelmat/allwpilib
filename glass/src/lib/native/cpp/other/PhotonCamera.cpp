...
struct FrameData {
  ...
  void DrawObject(ImDrawList* drawList, PhotonCameraObjectModel& objModel,
                  const frc::Pose2d& pose) const;
  void DrawGroup(ImDrawList* drawList, PhotonCameraObjectGroup& group,
                 const frc::Pose2d& pose) const;
...
void glass::DisplayPhotonCameraSettings(PhotonCameraModel* model) {
...
void FrameData::DrawObject(ImDrawList* drawList,
                           PhotonCameraObjectModel& objModel,
                           const frc::Pose2d& startPose) const {
  auto endPose = frc::Pose2d{frc::Translation2d{units::meter_t{objModel.GetX()}, units::meter_t{objModel.GetY()}}, 0_deg};
...
void FrameData::DrawGroup(ImDrawList* drawList, PhotonCameraObjectGroup& group,
                          const frc::Pose2d& pose) const {
  group.ForEachObject(
      [&](auto& objModel) { DrawObject(drawList, objModel, pose); });
}

void glass::DisplayPhotonCamera(PhotonCameraModel* model,
...
void PhotonCameraView::Display() {
  if (ImGui::BeginPopupContextItem()) {
    DisplayPhotonCameraSettings(m_model);
    ImGui::EndPopup();
  }
  DisplayPhotonCamera(m_model, ImGui::GetWindowContentRegionMax() -
